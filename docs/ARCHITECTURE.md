...
**The one server surface.** Closing an account is the only thing the browser
cannot do, because removing the `auth.users` row needs the service-role key:
`supabase/functions/close-account/` is an Edge Function that reads the caller from
the Auth API, runs `delete_my_data()` with the caller's own token so RLS still
decides, and only then deletes the row. It imports nothing — the one-SDK-importer
rule is about importers, and a function is a second runtime rather than a second
one — and `./scripts/meditaur cloud --yes` deploys it with the migrations, so a
release cannot ship the caller without the other half.
...
What that means for the two things that do need mail: a password reset
([ROADMAP.md](./ROADMAP.md), the `P2` password-reset row's gate) and an address
change both send from the platform, so both wait on an allowlisted SMTP in
[DEPENDENCIES.md](./DEPENDENCIES.md). A password change by a reader who is already
signed in needs none, and neither do passkeys — which is item 6's brief.
...
`archivedAt`, which is the reader’s own undo and drawable in the Archive. A **plan**
carries no such mark yet: it is versioned by `revision` alone, so its tombstone is
the plan repository’s decision in the slice that adds its cloud adapter.
...
What sync still owes, in the order it has to be done, is in
[ROADMAP.md](./ROADMAP.md): the catalogue's refetch-per-mutation (`P2 · 4`) first, then
the cloud adapter behind the catalogue’s port, then the protocol that pushes and
pulls, then the wiring and the copy. It is also where the
