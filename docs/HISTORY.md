...
| `H2` | Password reset | Closed 2026-09-22 — the owner chose a hand-run reset, so no mailer is integrated ([DECISIONS.md](./DECISIONS.md) §11); the copy and the panel action are **item 36**, and the address-change half is **item 6** |
...
code: an Edge Function that resolves the caller from the Auth API with the caller's
