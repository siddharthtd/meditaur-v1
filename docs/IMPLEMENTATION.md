...
- The plan strip's cards are a fixed `w-52 min-w-52` (the app's root font is 18px, so that is 234px), with the card's name as the drag handle and `Remove` beside it. **There is no cool-off block and no block kind** (the owner's round 15, 2026-09-19): a card's handle says the meditation's name with its type under it, and the whole strip has one **`Add meditation block`**, which opens the same `PickerPage` a card's `Meditation` field does — every live meditation, every live type, the type beside the name. `PlanBlock` has no `type`, and `compilePlan` refuses a block that names no meditation. The words `· drag` were removed on the owner's round 5, because every card said the same word and it set the card's width; the drag instruction lives in the handle's `aria-label` instead. Under the handle the card draws **one row per stage** — the stage's label above its `M`/`S` wheels, stacked so the card keeps its width — and the run screen's pre-Start header draws the same rows for the block on screen.
...
- **What the gate costs, and the three caches that keep it there** (measured 2026-09-18 on a 12-core machine): `check` is 167s cold and 38s warm, the two live databases together are 21s, `build` is 114s, e2e is 3m11s. The suite is not slow because the tests are heavy — the 66 e2e tests are 696s of test time between them, and three tests that only render a page cost 14–16s each. It is slow because of startup, so the caches are the whole budget: keep turbo's `.turbo` (`scripts/clean.sh` sweeps it; `clean-run.sh` must not, and deleting it before a gate cost 134s of typecheck and lint), keep `*.tsbuildinfo` (`tests/tsconfig.json` is `incremental`: 48s → 10s) and keep `eslint --cache` (44s → 13s). `turbo.json`'s `build` task declares `NODE_ENV` and both `NEXT_PUBLIC_SUPABASE_*` names as `env` because Next inlines them into the bundle — without that, keeping `.turbo` would be a false cache hit waiting to happen. e2e's worker count comes from `os.availableParallelism()` (6 here, 3 on a 4-vCPU CI runner, `PLAYWRIGHT_WORKERS` overrides it): the suite is bound by the one `next dev` server, and 3 workers spend 5m41s on it where 6 spend 3m11s.
- **The e2e suite compiles its routes before any test starts.** The e2e image ships no `.next`, and the suite's server is `next dev`, so a route would otherwise be compiled on the first request of every run — inside whichever test arrives first, with three workers arriving at once. That is what failed the Display test on the merge: its own budget was paying for a compile. `tests/e2e/warmup.ts` is the config's `globalSetup` and visits every route the specs visit, **in series** (~40s, and the whole of `globalSetup`) — serial is measured, not assumed: three pages at a time was 43.9s with the sum of the ten waits going 37.7s → 124.9s, because `next dev` compiles through one module graph and a second page queues behind the first rather than compiling alongside it. Playwright starts the web server as a plugin *before* global setups, which is what makes any of it possible. **Add a route to `ROUTES` in that file when a spec starts visiting a new one** — it costs its own compile and nothing more. Warming covers servers rather than a worker's first client navigation, so a wait on a screen that was just compiled wants its own timeout — 15s is the `expect` default and it is not enough on a cold container (`plans.spec.ts`'s `/run/` wait carries a 60s one for exactly that reason, after it flaked on the default and passed on retry).
...
- **Closing an account is one order, owned by the application, not by a screen**:
  ...
  `supabase/functions/close-account/`, which carries **no imports at all** on
  purpose: the one-SDK-importer rule above is about importers, and a function is a
  second runtime rather than a second importer. `./scripts/meditaur cloud --yes`
  deploys it with the migrations, so the caller and the thing it calls ship
  together, and the platform verifies the JWT before it runs. Nothing typechecks or
  lints it — Deno is not a runtime this repo's toolchain has — so the hosted live
  suite is what proves it, and that gap is the register's `P4 · 34`. That suite is
  also the only one that *can*: the local stack serves Postgres and Auth and no
  `functions serve`, so `tests/integration/account-close.test.ts` skips on the local
  target and runs on the hosted one. The data half is still covered locally, by
  `rls.test.ts` calling `delete_my_data()` against the stack.
...
- **A block reads its own meditation's sentences.** `CompiledBlock.affirmations:
  string[]` is filled at compile time from the live intentions of the block's own
  meditation, in `sortOrder`, symbol-carrying rows included — so a Protection stage
  reads Protection's sentences rather than every sentence in the workspace. A
  snapshot keeps saying what the session said even if the reader edits afterwards,
  and nothing references a sentence by id, which is why removing one takes nothing
  else with it.
