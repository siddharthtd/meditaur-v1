...
/**
 * One record's own view, reached the way a reader reaches it now.
 *
 * The library's `Edit` lands in the Database grid, so the *page* a record still has
 * is the one behind its row's `Open` — for the fields the grid has no column for.
 * A row is found by its name cell, because a value in an `<input>` is not text and
 * `hasText` cannot see it.
 */
async function openRecordFromGrid(page: Page, table: string, name: string): Promise<void> {
...
test("adding a meditation lands in the Database, in a row to fill in", async ({ page }) => {
  ...
  // And `Edit` goes to the grid with this row's name live, not to a page.
...
test("chakra meditations show the chakra-only fields", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Save", exact: true }).click();
  await expect(page.getByText("Saved.")).toBeVisible();
  await openRecordFromGrid(page, "meditation", "Navel");
...
test("escape goes back from the record editor", async ({ page }) => {
  await page.goto("/library");
  await openRecordFromGrid(page, "meditation", "Root Chakra");
  await expect(page.getByRole("textbox", { name: "Meditation name" })).toBeVisible();
  await page.keyboard.press("Escape");
  // The record view is a screen *inside* the Database, so Escape hands back to the
  // grid it was opened from — not to the library's browse list.
  await expect(page.getByRole("button", { name: "Add a column at the end" })).toBeVisible();
});
...
test("a trip to the binaural config writes the grid's draft rather than dropping it", async ({
  ...
  // is written on the way through and the other screen comes back with `Back` to a
  // grid that still has the name in it.
  await page.getByRole("button", { name: "Back" }).click();
